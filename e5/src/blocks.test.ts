...
describe("Function reverseBlocks", () => {
  it("reverseBlocks returns error if blockSize is greater than arrays length", () => {
    expect(() => reverseBlocks([0, 1, 2], 4)).toThrow(
  ...
  it.each([
    {
      arr: [],
      blockSize: 0,
      expected: "blockSize has to be greater than zero",
    },
    {
      arr: [],
      blockSize: -5,
    ...
    ({ arr, blockSize, expected }) => {
      expect(() => reverseBlocks(arr, blockSize)).toThrow(expected);
    }
  );
});
